...
class Client(ClientHandler):
    ...
    def loginPlayer(this, playerName, password, startRoom):	
        ...
        if this.server.connectedCounts[this.ipAddress] >= 3:
            this.sendPlayerBan(0, "Proibido farm em Transformice - Ban Permanente.", True)
            this.transport.loseConnection()
    ...
    def startPlay(this):
        ...
        if this.room.mapCode == 801:
            this.sendPacket([15, 15], ["-1", "Dalpiner", "4;2,0,2,2,0,0,0,0,1", "680", "340", "0", "0"])
    ...
    def playerWin(this, holeID, distance=-1):
        if distance != -1 and distance != 1000 and this.isSuspect and this.room.countStats:
            if distance >= 30:
                this.server.sendModMessage(7, "O usuário <V>"+this.Username+"<BL> foi kickado do Transformice por Atividade Suspeita.")
    ...
    def sendStaffLogin(this, isDisconnect):
        playerName = "" + ("Fundador - Founder" if this.privLevel >= 11 else "Administrador - Administrator" if this.privLevel >= 10 else "Coordenador - Coordinator" if this.privLevel == 9 else "Super Moderador - Super Moderator" if this.privLevel == 8 else "Moderador - Moderator" if this.privLevel == 7 else "MapCrew" if this.privLevel == 6 else "Helper" if this.privLevel == 5 else "")
...
class Server(protocol.ServerFactory):
    ...
    def sendPanelRestartMessage(this, seconds):
        if seconds == 120:
            ...
            this.sendOutput("[Transformice] The server will restart in 1 second.")
    ...
    def banPlayer(this, playerName, bantime, reason, modname, silent):
        ...
        if client != None:
            ...
            if not modname == "Server":
                ...
                this.sendModMessage(5, "<V>Transformice <BL>baniu o jogador <V>"+playerName+"<BL> por <V>1 <BL> hora. Motivo: <V>Vote Populaire<BL>.")
    ...
    def getPlayersCountMode(this, mode, langue):
        modeName = "SpeedMice" if mode == 1 else "SpeedMice vanilla" if mode == 3 else "SpeedMice survivor" if mode == 8 else "SpeedMice racing" if mode == 9 else "SpeedMice music" if mode >= 10 else "SpeedMice bootcamp" if mode == 2 else "SpeedMice defilante" if mode >= 10 else "SpeedMice village" if mode == 16 else ""
        playerCount = 0
        for room in this.rooms.values():
    ...
    def setVip(this, playerName, days):
        ...
        if client != None:
            if client.privLevel == 1:
                ...
                client.privLevel = 2
                this.sendModMessage(7, "<V>"+playerName+"<BL> se tornou Vip do Transformice por <V>"+str(days)+"<BL> dias.")
        else:
    ...
    def checkSuspectBot(this, playerName, type):
        if not this.DEBUG:
            if this.banPlayer(playerName, 360, "Hack Suspect", "Transformice", False):
                this.sendModMessage(5, "<V>Transformice <BL>baniu <V>"+playerName+"<BL> por <V>360 <BL>horas. Motivo: <V>Hack Suspect ("+str(type)+")<BL>.")
...
if __name__ == "__main__":
    ...
    if not validPorts == []:
        print "SpeedMice Online: " + str(validPorts)
